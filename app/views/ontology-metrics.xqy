...
let $descriptions :=  $doc/rdf:RDF/rdf:Description
return
<metrics>
   <count-descriptions>{count($descriptions)}</count-descriptions>
   <top-concepts>{count($doc//skos:hasTopConcept)}</top-concepts>
</metrics>